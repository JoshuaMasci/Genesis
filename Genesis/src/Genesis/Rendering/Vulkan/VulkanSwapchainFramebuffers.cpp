...
VulkanSwapchainFramebuffers::VulkanSwapchainFramebuffers(VulkanDevice* device, VulkanSwapchain* swapchain, VulkanSurface* surface, VulkanAllocator* allocator)
{
	this->device = device->get();
	this->allocator = allocator;

	this->render_pass = surface->getRenderPass();

	VkFormat swapchain_image_format = surface->getFormat().format;
	VkFormat swapchain_depth_format = surface->getDepthFormat();

	uint32_t swapchain_image_count = swapchain->getImageCount();
