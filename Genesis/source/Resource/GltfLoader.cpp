			...
			for (size_t primitive_index = 0; primitive_index < mesh.primitives.size(); primitive_index++)
			{
				...
				// Vertices
				{
					...
					for (size_t vertex_index = 0; vertex_index < position_accessor.count; vertex_index++)
					{
						...
						vertex.joints = has_skin ? vector4F(glm::make_vec4(&buffer_joints[vertex_index * joint_byte_stride])) : vector4F(0.0f);
						vertex.weights = has_skin ? glm::make_vec4(&buffer_weights[vertex_index * weight_byte_stride]) : vector4F(0.0f);
						// Fix for all zero weights
						if (glm::length(vertex.weights) == 0.0f)
						{
							vertex.weights = vector4F(1.0f, 0.0f, 0.0f, 0.0f);
						}
