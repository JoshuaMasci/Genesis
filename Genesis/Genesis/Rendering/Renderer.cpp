...
void Renderer::endLayer()
{
...

	{
		...
		matrix4F model = model_transform.getModelMatrix();
...
void Renderer::drawModel(Shader shader, PipelineSettings& settings, MeshTemp& mesh, void* uniform_const, uint32_t uniform_const_size)
{
	GENESIS_ENGINE_ASSERT_ERROR((this->command_buffer != nullptr), "Command buffer not valid");
